...
/*  Copyright 2012  Agnel Waghela  (email : agnelwaghela@gmail.com)
...
*/
	...
	class Zocial {
		...
		function zocial_wp_head() {
			...
			wp_enqueue_script( 'zocial_tinymce_popup', get_bloginfo('wpurl') . '/wp-includes/js/tinymce/tiny_mce_popup.js' );
		}
		...
		function filter_mce_button( $buttons ) {
			// add a separation before our button, here our button's id is "zocial"
			array_push( $buttons, 'separator', 'zocial' );
			return $buttons;
		}
		...
		function filter_mce_plugin( $plugins ) {
			...
			$params = '?zocialpop=' . rawurlencode( $url ) . '&pluginUrl='. rawurlencode( $this->pluginUrl );
			$plugins['zocial'] = $this->pluginUrl."zocial.mce_plugin.php{$params}";
			//$plugins['zocial'] = $this->pluginUrl. 'zocial.editor.js';
			return $plugins;
		}
		...
		function zocial_shortcode( $atts, $content = 'Sign in with' ) {
			...
			if ( $icon != 'true' ) {
				$c =  '<a href="'.$href.'" class="zocial '.$s.'">'.$content.'</a>';
			}
			else {
				$c = '<a href="'.$href.'" class="zocial icon '.$s.'">Sign in with'.$s.'</a>';
			}
